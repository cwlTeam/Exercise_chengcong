...
{
    QString mapName;
    QFile mapFile("maps.txt");                      //以只读形式打开此文件
    if(mapFile.open(QIODevice::ReadOnly))           //分别读取地图的名称个四个经纬度信息
    {
...
void MapWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    //地图坐标
    QPoint latLon = mapTomap(scenePoint);
...
void MapWidget::slotZoom(int value)
{
    ...
    if(value>zoom)
    {
        s=pow(1.01,(value-zoom));               //放大
    }
    else{
        s=pow(1/1.01,(zoom-value));             //缩小
    }
    scale(s,s);
    zoom = value;
}
